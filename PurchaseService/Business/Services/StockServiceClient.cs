...
namespace PurchaseService.Business.Services
{
    public class StockServiceClient : IStockServiceClient
    {
        ...
        public async Task RegisterPurchaseMovementsAsync(int userId, int warehouseId, List<(int articleId, decimal quantity, decimal unitCost)> items, string reference, int? dispatchId)
        {
        ...
        public async Task RegisterPendingStockConsolidatedAsync(RegisterPendingStockInputDTO dto, int userId)
        {
            var client = _httpClientFactory.CreateClient();
            ...
            var url = $"{_stockBaseUrl.TrimEnd('/')}/pending-entry/register";
            var response = await client.PostAsJsonAsync(url, dto);
