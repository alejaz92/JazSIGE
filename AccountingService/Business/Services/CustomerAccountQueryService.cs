...
namespace AccountingService.Business.Services
{
    public class CustomerAccountQueryService : ICustomerAccountQueryService
    {
        ...
        public async Task<BalancesDTO> GetBalancesAsync(int customerId, CancellationToken ct = default)
        {
            ...
            // 2) Créditos: NC activas (disponible) + recibos no aplicados
            ...
            var creditNotesAvailable = creditDocs.Sum(cd =>
            {
                appliedByCreditDoc.TryGetValue(cd.Id, out var applied);
                var available = cd.TotalBase - applied;
                return available > 0 ? decimal.Round(available, 2, MidpointRounding.ToEven) : 0m;
            });

            // Recibos no aplicados (igual que hoy)
            ...
            var credits = decimal.Round(creditNotesAvailable + advances, 2, MidpointRounding.ToEven);
